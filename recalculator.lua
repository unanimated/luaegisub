...
function multiply(subs, sel)
    ...
    for x, i in ipairs(sel) do
        if res.byline then count=x*c linec=x else count=c linec=1 end
	line=subs[i]
	text=line.text
	...
	if res.fsp and not text:match("\\fsp") and spac~=0 then text=text:gsub("^({\\[^}]*)}","%1\\fsp"..spac.."}") end
	    ...
	    if res.fscy then neg=0 text=text:gsub("\\fscy([%d%.]+)",function(a) return "\\fscy"..calc(tonumber(a)) end) end
	    ...
	    if res.ybord then neg=0 text=text:gsub("\\ybord([%d%.]+)",function(a) return "\\ybord"..calc(tonumber(a)) end) end
	    if res.xshad then neg=1 text=text:gsub("\\xshad([%d%.%-]+)",function(a) return "\\xshad"..calc(tonumber(a)) end) end
	    if res.yshad then neg=1 text=text:gsub("\\yshad([%d%.%-]+)",function(a) return "\\yshad"..calc(tonumber(a)) end) end
	    if res.frx then neg=1 text=text:gsub("\\frx([%d%.%-]+)",function(a) return "\\frx"..calc(tonumber(a)) end) end
	    if res.fry then neg=1 text=text:gsub("\\fry([%d%.%-]+)",function(a) return "\\fry"..calc(tonumber(a)) end) end
	    ...
	    if res.fay then neg=1 text=text:gsub("\\fay([%d%.%-]+)",function(a) return "\\fay"..calc(tonumber(a)) end) end
		...
		if res.mov2 then b=calc(tonumber(b)) end
		if res.mov3 then c=calc(tonumber(c)) end
		if res.mov4 then d=calc(tonumber(d)) end
	    ...
	    return "\\org("..a..","..calc(tonumber(b))..")" end) end
	    ...
	    return "\\fad("..a..","..calc(tonumber(b))..")" end) end
	    ...
	    return "\\t("..a..","..calc(tonumber(b)).."," end) end
...
function recalculator(subs, sel)
	dialog_config=
	{
	    ...
	    {x=0,y=2,width=2,height=1,class="label",label="Rounding:",},
	    {x=2,y=2,width=3,height=1,class="intedit",name="rnd",value=default_rounding,min=0,hint="How many decimals should be allowed"},
	    
	    {x=0,y=3,width=1,height=1,class="checkbox",name="fscx",label="fscx",value=true,},
	    {x=1,y=3,width=1,height=1,class="checkbox",name="fscy",label="fscy",value=true,},
	    {x=2,y=3,width=1,height=1,class="checkbox",name="fs",label="fs",value=false,},
	    {x=3,y=3,width=1,height=1,class="checkbox",name="fsp",label="fsp",value=false,},
	    {x=4,y=3,width=1,height=1,class="checkbox",name="blur",label="blur",value=false,},
	    {x=5,y=3,width=1,height=1,class="checkbox",name="be",label="be",value=false,},

	    {x=0,y=4,width=1,height=1,class="checkbox",name="bord",label="bord",value=false,},
	    {x=1,y=4,width=1,height=1,class="checkbox",name="shad",label="shad",value=false,},
	    {x=2,y=4,width=1,height=1,class="checkbox",name="xbord",label="xbord",value=false,},
	    {x=3,y=4,width=1,height=1,class="checkbox",name="ybord",label="ybord",value=false,},
	    {x=4,y=4,width=1,height=1,class="checkbox",name="xshad",label="xshad",value=false,},
	    {x=5,y=4,width=1,height=1,class="checkbox",name="yshad",label="yshad",value=false,},

	    {x=0,y=5,width=1,height=1,class="checkbox",name="frz",label="frz",value=false,},
	    {x=1,y=5,width=1,height=1,class="checkbox",name="frx",label="frx",value=false,},
	    {x=2,y=5,width=1,height=1,class="checkbox",name="fry",label="fry",value=false,},
	    {x=3,y=5,width=1,height=1,class="checkbox",name="fax",label="fax",value=false,},
	    {x=4,y=5,width=1,height=1,class="checkbox",name="fay",label="fay",value=false,},
	    {x=5,y=5,width=1,height=1,class="checkbox",name="kara",label="kara",value=false,hint="k/kf/ko. only the first one in the line."},
	    ...
	    {x=2,y=6,width=1,height=1,class="checkbox",name="orgx",label="org x",value=false,},
	    {x=3,y=6,width=1,height=1,class="checkbox",name="orgy",label="org y",value=false,},
	    {x=4,y=6,width=1,height=1,class="checkbox",name="fad1",label="fad 1",value=false,},
	    {x=5,y=6,width=1,height=1,class="checkbox",name="fad2",label="fad 2",value=false,},

	    {x=0,y=7,width=1,height=1,class="checkbox",name="mov1",label="move1",value=false,},
	    {x=1,y=7,width=1,height=1,class="checkbox",name="mov2",label="move2",value=false,},
	    {x=2,y=7,width=1,height=1,class="checkbox",name="mov3",label="move3",value=false,},
	    {x=3,y=7,width=1,height=1,class="checkbox",name="mov4",label="move4",value=false,},

	    {x=4,y=7,width=2,height=1,class="checkbox",name="allpos",label="all pos/move/org",value=false,hint="same as all 8 checkboxes. \naffects only existing tags."},

	    {x=0,y=8,width=1,height=1,class="checkbox",name="clipx",label="clip x",value=false,},
	    {x=1,y=8,width=1,height=1,class="checkbox",name="clipy",label="clip y",value=false,},
	    {x=2,y=8,width=1,height=1,class="checkbox",name="drawx",label="draw x",value=false,},
	    {x=3,y=8,width=1,height=1,class="checkbox",name="drawy",label="draw y",value=false,},
	    {x=4,y=8,width=1,height=1,class="checkbox",name="ttim1",label="\\t 1",value=false,hint="\\t timecode 1"},
	    {x=5,y=8,width=1,height=1,class="checkbox",name="ttim2",label="\\t 2",value=false,hint="\\t timecode 2"},

	    {x=0,y=9,width=4,height=1,class="checkbox",name="byline",label="multiply/add more with each line",value=false,},
	    {x=4,y=9,width=2,height=1,class="checkbox",name="anchor",label="anchor clip",value=false,hint="anchor clip with Multiply"},
	} 	
	    chk=","..checked..","
	    chk=chk:gsub(" *, *",",") :gsub("\t","\\t")
